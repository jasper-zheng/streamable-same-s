...
try:
    # cached_conv (RAVE) is only consulted to read MAX_BATCH_SIZE for the optional
    # multichannel (mc.nn~) test path below. SAME-S uses no cached convolutions, so
    # the package is not required; fall back to 1 (skips only the mc test).
    ...
    def register_method(
        ...
        if test_method:
            ...
            if MAX_BATCH_SIZE > 1:
                ...
                logging.info(
                    f"Skipping method {method_name} with mc.nn~ API as MAX_BATCH_SIZE={MAX_BATCH_SIZE}"
                )
        else:
